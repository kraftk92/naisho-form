...
// run after DOM is ready (works whether script is in <head> or end of <body>)
const ready = (fn) =>
...
ready(() => {
  ...
  if (!shell) {
    console.error("Naisho form mount point #naisho-form not found.");
    return;
  }
  ...
  // now these exist
  const form       = shell.querySelector("#naisho-form-inner");
  ...
  if (!dateInput) {
    console.error("Date input not found after mount.");
    return;
  }
  ...
  if (phoneInput){
    ...
    requestAnimationFrame(sync); // normalize browser autofill on load
  }
